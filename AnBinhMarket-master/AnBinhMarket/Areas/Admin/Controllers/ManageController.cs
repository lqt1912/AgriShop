...
namespace AnBinhMarket.Areas.Admin.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public IActionResult DoanhThuHomNay()
        {
            var todayHds = db.HoaDons
                .Include(x => x.GioHang)
                .ThenInclude(x => x.ChiTietGioHangs)
                ...
                .Where(x => x.NgayTao.Date == DateTime.Today.Date
                && x.TrangThai.Equals("Đã giao")
                && !x.IsDeleted);
            var result = new List<HoaDonViewModel>();
            foreach (var hoaDon in todayHds)
            {
                decimal thanhTien = 0;
        ...
        public IActionResult ThongKeTrongNam(DateTime? fromDate, DateTime? toDate)
        {
            ...
            {
                hd_trong_nam = receipts.Where(x=>x.NgayTao.Date >= fromDate.Value.Date && x.NgayTao.Date <= toDate.Value.Date).ToList();
